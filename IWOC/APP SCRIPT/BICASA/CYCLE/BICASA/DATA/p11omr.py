            ...
            for i in range(a,b):
                ...
                if 'PROFIT PAID' in line:
                    ...
                    if 'TOTAL DEBIT' in line1:
                        msg_total_debit.append(line1[40:50])     
                        flag1='True'  
                    if 'TOTAL CREDIT' in line2:
                        msg_total_credit.append(line2[40:50])     
                        flag2='True'  
...
for i in range(0,Total_non_email_account):
    ...
    # --------------1st and 10th line on left omr and 7th line in right omr    
        if page % 2 != 0:
            parity += 2
            rparity += 1
            pdf.set_line_width(0.8)
            pdf.line(rbs1,rbs2,rbs3,rbs4)
            pdf.line(a1,b1,c1,d1)
            pdf.line(a10,b10,c10,d10)
        ...
        if page == reoc:
            rparity += 1
            pdf.set_line_width(0.8)
            pdf.line(reoc1,reoc2,reoc3,reoc4) 
            pdf.set_font("arial", "B",20)
            pdf.set_text_color(0,0,0)
            reoc += 6


        # print right side BS1 line
        if page %10 == 3:
            rparity += 1
            pdf.set_line_width(0.8)
            pdf.line(bsr1,bsr2,bsr3,bsr4)
            pdf.set_font("arial", "B",20)
            pdf.set_text_color(0,0,0)

        # print right side BS2 line    
        if page %10 == 5:
            rparity += 1
            pdf.set_line_width(0.8)
            pdf.line(bsr1,bsr2-2,bsr3,bsr4-2)
            pdf.set_font("arial", "B",20)
            pdf.set_text_color(0,0,0)
            
        # print right side BS1 and BS2 line
        if page %10 == 7:
            rparity += 2
            pdf.set_line_width(0.8)
            pdf.line(bsr1,bsr2,bsr3,bsr4)
        ...
        # print right side BS4 line
        if page %10 == 9:
            rparity += 1
            pdf.set_line_width(0.8)
            pdf.line(bsr1,bsr2-4,bsr3,bsr4-4)
            pdf.set_font("arial", "B",20)
            pdf.set_text_color(0,0,0)
        
        if rparity % 2 == 0 and page%2!=0:
            pdf.set_line_width(0.8)
        ...
        #     #--------------------------------------------INSERTING 1ST TABLE DATA OF 1ST PAGE-----------------
            ...
            if msg_total_debit[i] == 'Test':
                ...
                pdf.cell(30)
                pdf.cell(20,h+20,txt = str(total_debit[i][j]))
                ...
                pdf.cell(30)
                pdf.cell(20,h+25,txt = str(total_credit[i][j]))
