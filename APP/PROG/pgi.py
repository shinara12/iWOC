...
def PGI(request,files1):
    ...
    def barcode(acc_no,count):
        ...
        my_code.save(r"E:\IWOC\barcode_fo\%s" %(new_code))


        img = Image.open(r"E:\IWOC\barcode_fo\%s.png" %(new_code))
        imgCropped = img.crop(box=(4,10,1080,200))
        imgCropped.save(r"E:\IWOC\barcode_fo\%s.png" %(new_code))
    ...
    for i in range(0,number_of_rows):
        ...
        pdf.ln(1)
        pdf.cell(10,146, txt = 'JABATAN KHIDMAT PENGURUSAN &')
        pdf.cell(140)
        pdf.cell(10,146,txt = 'TANPA PRASANGKA')

        pdf.ln(1)
        pdf.cell(10,152,txt = 'PEMULIHAN HUTANG')

        pdf.ln(1)
        pdf.cell(10,158,txt = 'BANK SIMPANAN NASIONAL')
        ...
        pdf.cell(10,164,txt ="(")
        pdf.cell(-8)
        pdf.cell(12,164,txt = str(data['br_code'][i]))
        pdf.cell(-7)
        pdf.cell(20,164,txt = ")")
        ...
        c += 1
        pdf.ln(1)
        pdf.image(r'E:\IWOC\APP SCRIPT\PGI\CYCLE\PGI\DATA\BSN_Reminder-02.jpg', 12,154,180)

        pdf.ln(1)
        # pdf.image('E:\IWOC\SOURCE\sampleqcode.PNG',40,210,60)
        new_code  = 'b' + str(count) 
        pdf.image(r'E:\IWOC\barcode_fo\%s.png'%(new_code),40,207,60)
