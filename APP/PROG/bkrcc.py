...
def BKRCC(request,files1):
    ...
    #-----------------------INSERTING DATA OF 8 TH TABLE -----------------------
        ...
        pdf.cell(10,39,txt = 'Anda kini boleh memilih untuk menebus mata Ganjaran Rakyat anda untuk dikreditkan ke Akaun Semasa-i/Simpanan-')
        pdf.ln(1)
        pdf.cell(4)
        pdf.set_font('Arial', '', 9.8)
    ...
    # today = date.today()
    # count = 1
    # data = History.objects.create(JobIDX  = '125',Date = str(today),Start = str(start_time),Finish=str(end_time),User = 'Admin',Product = 'KKK', File= 'BKRCC',Status = 'Enable',Log = str(total_clients),ActiveCount = str(total_clients),Page = str(total_clients),Impression  = 'Done')    
