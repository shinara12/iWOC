...
def REMINDER2(request,files1):
    ...
    postcode = data['POSTCODE'].to_list()
    postcode_final = []
    for i in postcode:
        i = str(i)
        k = i.split()
    ...
    def barcode(acc_no,count):
        ...
        my_code.save(r"E:\IWOC\EXTRA\barcode_for_rem2\%s" %(new_code))

        from PIL import Image

        img = Image.open(r"E:\IWOC\EXTRA\barcode_for_rem2\%s.png" %(new_code))
        imgCropped = img.crop(box=(4,10,1080,200))
        imgCropped.save(r"E:\IWOC\EXTRA\barcode_for_rem2\%s.png" %(new_code))
        # imgCropped.show()
    ...
    for i in range(0,Total_accounts):
        ...
        if pd.notna(df['POSTCODE'][i]):
            ...
            pdf.ln(1)
            pdf.cell(60)
            pdf.cell(10,h,txt = str(postcode_final[i]))
            pdf.cell(-1)
        ...
        pdf.image(r'E:\IWOC\APP SCRIPT\REMINDER2\CYCLE\REMINDER2\DATA\BSN_Reminder-02.jpg',20,146,80)
        pdf.image(r'E:\IWOC\APP SCRIPT\REMINDER2\CYCLE\REMINDER2\DATA\BSN_Reminder-02 - Copy.jpg', 145,146,45)

        pdf.ln(1)
        new_code  = 'b' + str(count)
        pdf.cell(11)
        pdf.image(r'E:\IWOC\EXTRA\barcode_for_rem2\%s.png'%(new_code),32,212,50)
        ...
        pdf.cell(10,420,txt = str(barcode_no1))
        

        pdf.set_font('times','',7)
        pdf.cell(26)
        pdf.cell(10,420,txt = '00' + str(count))


        pdf.set_font('times','',7)
        pdf.cell(-5)
        pdf.cell(10,420,txt ='(' + str(postcode[i]) + ')')

        
        pdf.set_font('times','',7)
        pdf.cell(-5)
        pdf.cell(10,420,txt =' -P02 - 01')
